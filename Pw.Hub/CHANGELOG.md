...
## [0.1.1] — 2025-11-03
Небольшое обновление UX и стабильности после 0.1.0: улучшено сохранение аккаунта, сохранение состояния дерева навигации при редактировании, доработано окно аргументов запуска (тип "аккаунты"). Также в API добавлены метрики Telegram через OpenTelemetry.

### Добавлено
- API (`pw.modules.api`): метрики OpenTelemetry для Telegram:
  - `telegram.linked` — успешные привязки Telegram.
  - `telegram.unlinked` — успешные отвязки Telegram.
  - `telegram.messages.sent` — успешные отправки сообщений в Telegram.

### Изменено
- Окно аргументов запуска (тип "аккаунты"):
  - Отображение аккаунтов по отрядам с заголовком-"группой" и три‑состоянием.
  - Вертикальная раскладка элементов вместо горизонтальной.
  - Кнопки быстрого действия: "Выбрать все" и "Снять все".
  - Клик по названию отряда ведёт себя как по чекбоксу заголовка (массовое выделение/снятие).

### Исправлено
- Окно редактирования аккаунта:
  - Кнопка «Сохранить» действительно сохраняет изменения в БД, включая "персонажа по умолчанию" по серверам.
  - После сохранения не сбрасывается раскрытие и текущий выбор в дереве навигации.
- Редактирование отряда: состояние дерева навигации (раскрытие/выбор) сохраняется — исключён полный `Reload()`.
- Окно аргументов запуска (тип "аккаунты"):
  - Возвращены чекбоксы (вместо toggle-кнопок), чтобы элементы корректно отображались; размещение вертикальное.

### Технические изменения
- `Pw.Hub`: без миграций БД; правки в `MainViewModel`, окнах редактирования и `ModuleArgsWindow`.
- `pw.modules.api`: расширен `ModuleMetrics` и подключена телеметрия через уже существующую конфигурацию OpenTelemetry (экспорт OTLP), без изменений схемы БД.
