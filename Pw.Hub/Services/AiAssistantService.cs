...
namespace Pw.Hub.Services
{
    ...
    public class AiAssistantService : IAiAssistantService, IDisposable
    {
        ...
        private void EnsureSystemPriming()
        {
            ...
            sb.AppendLine("1. ВСЕ ФУНКЦИИ ДОЛЖНЫ БЫТЬ ОБЪЯВЛЕНЫЕ СВЕРХУ, А НИЖЕ ПРИСВАИВАЙ ЗНАЧЕНИЯ!!!");
            sb.AppendLine("2. Используй асинхронные функции (…Cb) и колбэки");
            sb.AppendLine("3. Каждый колбэк - должен быть отдельной фукнцией");
            sb.AppendLine("4. Добавляй комментарии на русском");
            sb.AppendLine("5. Обрабатывай ошибки и пограничные случаи");
            sb.AppendLine("6. Понятные имена переменных, логируй этапы через Print()");
            sb.AppendLine("7. Если доступна Complete — вызывай в конце");
            sb.AppendLine("8. Перед обращением к элементу — дождись его появления (например, 1000 мс)");
            sb.AppendLine("9. Навигация разрешена только на домен pwonline.ru");
            sb.AppendLine("10. JS должен возвращать строку; массивы кодируй в строку и парси в Lua");
